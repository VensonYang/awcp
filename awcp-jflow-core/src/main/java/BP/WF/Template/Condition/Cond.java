...
public class Cond extends EntityMyPK {
	...
	protected boolean beforeUpdateInsertAction() {
		...
		// this.RunSQL("UPDATE WF_Node SET IsCCNode=1 WHERE NodeID IN (SELECT NodeID FROM WF_Cond WHERE CondType="
		// + (int)CondType.Node + ")");
		try {
	...
	public final void setFK_Attr(String value) {
		...
		BP.Sys.Frm.MapAttr attr = new BP.Sys.Frm.MapAttr(value);
	...
	public final void setOperatorValue(Object value) {
		this.SetValByKey(CondAttr.OperatorValue,
	...
	public final void DoUp(int fk_node) {
		...
		this.DoOrderUp(CondAttr.FK_Node, (new Integer(fk_node)).toString(),
				CondAttr.CondType, (new Integer(condtypeInt)).toString(),
				CondAttr.PRI);
	}
	...
	public final void DoDown(int fk_node) {
		int condtypeInt = this.getHisCondType().getValue();
		this.DoOrderDown(CondAttr.FK_Node, (new Integer(fk_node)).toString(),
				CondAttr.CondType, (new Integer(condtypeInt)).toString(),
				CondAttr.PRI);
	}
	...
	public boolean getIsPassed() {
		...
		if (this.en == null) {
			...
			try {
				...
				// this.Delete();
				return false;
				// throw new Exception("@在取得判断条件实体[" + nd.EnDesc + "], 出现错误:" +
				// ex.Message +
				// "@错误原因是定义流程的判断条件出现错误,可能是你选择的判断条件工作类是当前工作节点的下一步工作造成,取不到该实体的实例.");
			}
		}
		...
		if (this.getHisDataFrom() == ConnDataFrom.Stas) {
			...
			if (BP.WF.Glo.getOSModel() == OSModel.WorkFlow) {
				...
				sts.Retrieve("FK_Emp", BP.Port.WebUser.getNo());
				for (BP.Port.EmpStation st : BP.Port.EmpStations
						.convertEmpStations(sts)) {
					if (strs.contains("@" + st.getFK_Station() + "@")) {
						this.MsgOfCond = "@以岗位判断方向，条件为true：岗位集合" + strs
								+ "，操作员(" + BP.Port.WebUser.getNo() + ")岗位:"
			...
			if (BP.WF.Glo.getOSModel() == OSModel.BPM) {
				...
				sts.Retrieve("FK_Emp", BP.Port.WebUser.getNo());
				for (BP.GPM.DeptEmpStation st : DeptEmpStations.convertDeptEmpStations(sts)) {
					if (strs.contains("@" + st.getFK_Station() + "@")) {
						this.MsgOfCond = "@以岗位判断方向，条件为true：岗位集合" + strs
								+ "，操作员(" + BP.Port.WebUser.getNo() + ")岗位:"
								+ st.getFK_Station() + st.getFK_StationT();
			...
			this.MsgOfCond = "@以岗位判断方向，条件为false：岗位集合" + strs + "，操作员("
					+ WebUser.getNo() + ")岗位:" + strs1;
			return false;
		}
		...
		if (this.getHisDataFrom() == ConnDataFrom.Depts) {
			...
			sts.Retrieve("FK_Emp", WebUser.getNo());
			String strs1 = "";
			for (BP.Port.EmpDept st : EmpDepts.convertEmpDepts(sts)) {
				if (strs.contains("@" + st.getFK_Dept() + "@")) {
					this.MsgOfCond = "@以岗位判断方向，条件为true：部门集合" + strs + "，操作员("
							+ WebUser.getNo() + ")部门:" + st.getFK_Dept()
							+ st.getFK_DeptT();
					return true;
				}
				strs1 += st.getFK_Dept() + "-" + st.getFK_DeptT();
			}

			this.MsgOfCond = "@以部门判断方向，条件为false：部门集合" + strs + "，操作员("
					+ BP.Port.WebUser.getNo() + ")部门:" + strs1;
			return false;
		}
		...
		if (this.getHisDataFrom() == ConnDataFrom.Url) {
			...
			url = url.replace("@SDKFromServHost", (String) BP.Sys.SystemConfig
			...
			if (url.contains("&SID") == false) {
				url += "&SID=" + BP.Port.WebUser.getSID();
			}

			if (url.contains("&UserNo") == false) {
				url += "&UserNo=" + BP.Port.WebUser.getNo();
			}
			...
			if (!url.contains("http")) {
				...
				if (SystemConfig.getIsBSsystem()) {
					...
					if (url.contains("@AppPath")) {
						url = url.replace("@AppPath", "http://" + host
								+ BP.Sys.Glo.getRequest().getRemoteAddr());
					} else {
						url = BP.Sys.Glo.getRequest().getRemoteHost();
					}
				}

				if (!SystemConfig.getIsBSsystem()) {
					// 在cs模式下它的baseurl 从web.config中获取.
					String cfgBaseUrl = (String) SystemConfig.getAppSettings()
							.get("BaseUrl");
			...
			try {
				...
				if (text == null)
				// throw new Exception("@流程设计的方向条件错误，执行的URL错误:" + url +
				// ", 返回为null, 请检查设置是否正确。");
				{
					return false;
				}
				...
				throw new RuntimeException("@判断url方向出现错误:" + ex.getMessage()
						+ ",执行url错误:" + url);
			}
			// /#endregion 对url进行处理.
		}
		...
		if (this.getHisDataFrom() == ConnDataFrom.Paras) {
			...
			if (row.containsKey(key) == false) {
				try {
					...
					valPara = BP.WF.Glo.SendHTOfTemp.get(key).toString().trim();
			...
			// throw new Exception("@判断条件时错误,没有找到对应的表达式:" + exp + " Key=(" + key
			// + ") oper=(" + oper + ")Val=(" + val+")");
		}

		try {
			if (en.getEnMap().getAttrs().Contains(this.getAttrKey()) == false) {
				throw new RuntimeException("判断条件方向出现错误：实体：" + nd.getEnDesc()
						+ " 属性" + this.getAttrKey() + "已经不存在.");
			}

			this.MsgOfCond = "@以表单值判断方向，值 " + en.getEnDesc() + "."
					+ this.getAttrKey() + " ("
					+ en.GetValStringByKey(this.getAttrKey()) + ") 操作符:("
			...
			// ORIGINAL LINE: case "<>":
			if (this.getFK_Operator().trim().toLowerCase().equals("<>")) {
				if (!this.getOperatorValue().toString()
						.equals(en.GetValStringByKey(this.getAttrKey()))) {
					return true;
				} else {
					return false;
				}
			...
			// ORIGINAL LINE: case "=":
			else if (this.getFK_Operator().trim().toLowerCase().equals("=")) // 如果是
																				// =
			{
				if (this.getOperatorValue().toString()
						.equals(en.GetValStringByKey(this.getAttrKey()))) {
			...
			// ORIGINAL LINE: case ">":
				...
				if (en.GetValDoubleByKey(this.getAttrKey()) > Double
			...
			// ORIGINAL LINE: case "like":
				...
				if (en.GetValStringByKey(this.getAttrKey()).indexOf(
				...
				throw new RuntimeException("@没有找到操作符号("
						+ this.getFK_Operator().trim().toLowerCase() + ").");
			}
		} catch (RuntimeException ex) {
			Node nd23 = new Node(this.getNodeID());
			throw new RuntimeException("@判断条件:Node=[" + this.getNodeID() + ","
					+ nd23.getEnDesc() + "], 出现错误。@" + ex.getMessage()
					+ "。有可能您设置了非法的条件判断方式。");
		}
	}
	...
	public Map getEnMap() {
		...
		map.AddTBString(CondAttr.FK_Operator, "=", "运算符号", true, true, 0, 60,
				20);
		map.AddTBString(CondAttr.OperatorValue, "", "要运算的值", true, true, 0,
				4000, 20);
		map.AddTBString(CondAttr.OperatorValueT, "", "要运算的值T", true, true, 0,
				4000, 20);
