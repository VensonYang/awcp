...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		// entity 数据放进cash.
		/*
		 * warning java.util.ArrayList al = ClassFactory.GetObjects("BP.En.Entities");
		for (Entities ens : al)*/
		java.util.ArrayList<Entities> al = ClassFactory.GetObjects("BP.En.Entities");
		for (Entities ens : al)
		{
			Depositary where;
			try
			{
				where = ens.getGetNewEntity().getEnMap().getDepositaryOfEntity();
			}
			catch (RuntimeException ex)
			{
				Log.DefaultLogWriteLine(LogType.Info, "@在把数据放在内存时出现错误:" + ex.getMessage() + " cls=" + ens.toString());
				// 包含用户登陆信息的map 都不取它。 
				continue;
			}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		/*
		 * warning al = ClassFactory.GetObjects("BP.XML.XmlEns");
		for (BP.XML.XmlEns ens : al)*/
		java.util.ArrayList<XmlEns> als = ClassFactory.GetObjects("BP.XML.XmlEns");
		for (XmlEns ens : als)
		{
			try
			{
			...
			{
				Log.DefaultLogWriteLineError("@调度  " + ens.toString() + "出现错误: " + ex.getMessage());
			}
		}
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
			///#endregion
