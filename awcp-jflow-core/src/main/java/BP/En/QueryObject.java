...
public class QueryObject
{
	...
	*/
	...
	public QueryObject(Entities ens)
	{
		...
		ens.clear();
		this._ens = ens;
		this.HisDBType = this._ens.getGetNewEntity().getEnMap().getEnDBUrl().getDBType();
	}
	...
	*/
	...
	{
		this.setSQL("( " + BP.Sys.SystemConfig.getAppCenterDBLengthStr() + "( " + attr2Field(attr) + " ) " + exp + " '" + (new Integer(len)).toString() + "')");
	}
	/** 
	...
	public final String GenerPKsByTableWithPara(String pk, String sql, int from, int to)
	{
		...
		for (DataRow dr : dt.Rows)
		{
			...
			if (i > from)
			{
				...
				/*
				 * warning this.getMyParasR().Add("R" + paraI, dr[0].toString());*/
				if (i >= to)
				{
				...
				switch (map.getEnDBUrl().getDBType())
				{
					case Oracle:
						...
						else
						{
							if (top == 0)
							{
								sql = "SELECT * FROM ( SELECT  " + pk + " FROM " + map.getPhysicsTable() + " WHERE " + this._sql + " " + this._orderBy + "   )  WHERE ROWNUM <=" + pageSize;
							}
							else
							{
					...
					case Informix:
						...
						else
						{
							if (top == 0)
							{
								sql = "SELECT first " + pageSize + " " + this.getEn().getPKField() + " FROM " + map.getPhysicsTable() + " WHERE " + this._sql + " " + this._orderBy;
							}
							else
							{
								sql = "SELECT  " + this.getEn().getPKField() + " FROM " + map.getPhysicsTable() + " WHERE " + this._sql + " " + this._orderBy;
							}
						}
					...
					case MySQL:
						...
						else
						{
							if (top == 0)
							{
								sql = "SELECT  " + this.getEn().getPKField() + " FROM " + map.getPhysicsTable() + " WHERE " + this._sql + " " + this._orderBy + " LIMIT " + pageSize;
							}
							else
							{
								sql = "SELECT  " + this.getEn().getPKField() + " FROM " + map.getPhysicsTable() + " WHERE " + this._sql + " " + this._orderBy;
					...
					default:
						...
						if (this._sql.equals("") || this._sql == null)
						{
							if (top == 0)
							{
								sql = " SELECT top " + pageSize + "  [" + this.getEn().getPKField() + "] FROM " + map.getPhysicsTable() + " " + this._orderBy;
							}
							else
							{
								sql = " SELECT  [" + this.getEn().getPKField() + "] FROM " + map.getPhysicsTable() + " " + this._orderBy;
						...
						else
						{
							if (top == 0)
							{
								sql = "SELECT TOP " + pageSize + " [" + this.getEn().getPKField() + "] FROM " + map.getPhysicsTable() + " WHERE " + this._sql + " " + this._orderBy;
							}
							else
							{
								sql = "SELECT  [" + this.getEn().getPKField() + "] FROM " + map.getPhysicsTable() + " WHERE " + this._sql + " " + this._orderBy;
				...
				for (DataRow dr : dt.Rows)
				{
					...
					for (Attr attr : attrs)
					{
						...
						for (int i = 0; i < list.size(); i++) {
							...
							if(column.ColumnName.equals(attr.getKey().toLowerCase())){
