...
public class Cash {
	...
	public static String GetBillStr(String cfile, boolean isCheckCash) {
		...
		if (StringHelper.isNullOrEmpty(val)) {
			...
			if (cfile.contains(":")) {
				...
				file = SystemConfig.getPathOfDataUser() + File.separator + "CyclostyleFile" + File.separator + cfile;
			}
	...
	public static String[] GetBillParas_Gener(String cfile, Attrs attrs) {
		...
		if (haveError) {
			String myfile = SystemConfig.getPathOfDataUser() + File.separator + "CyclostyleFile" + File.separator
					+ cfile;
		...
		if (!msg.equals("")) {
			// String s = "@帮助信息:用记事本打开它模板,找到红色的字体.
