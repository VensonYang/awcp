...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	...
	{
		String sql = "SELECT OID, TITLE, RDT, Rec FROM  " + this.getGetNewEntity().getEnMap().getPhysicsTable() + " WHERE OID IN (   SELECT WorkID FROM WF_GenerWorkFlow WHERE FK_Node IN ( SELECT FK_Node FROM WF_GenerWorkerlist WHERE FK_Emp='" + WebUser.getNo() + "' AND FK_Flow='" + flow + "' AND WORKID=WF_GenerWorkFlow.WORKID )  )";
