...
public class WorkflowDefintionManager {
	 ...
	 *            流程编号
	 * @param nodes
	 *            节点信息，格式为:@NodeID=xxxx@X=xxx@Y=xxx@Name=xxxx@RunModel=1
	 * @param dirs
	 *            方向信息，格式为: @Node=xxxx@ToNode=xxx@X=xxx@Y=xxx@Name=xxxx
	 * @param labes
	 *            标签信息，格式为:@MyPK=xxxxx@Label=xxx@X=xxx@Y=xxxx
	 */
	public static String SaveFlow(String fk_flow, String nodes, String dirs,
			String labes) {
		try {
			...
			for (String dir : mydirs) {
				...
				enDir.setIsCanBack(ap
						.GetValBoolenByKey(DirectionAttr.IsCanBack));
			...
			for (String nd : nds) {
				...
				sql = "UPDATE WF_Node SET X=" + ap.GetValStrByKey("X") + ",Y="
						+ ap.GetValStrByKey("Y") + ", Name='"
						+ ap.GetValStrByKey("Name") + "' WHERE NodeID="
						+ ap.GetValIntByKey("NodeID");
				DBAccess.RunSQL(sql);
			}
	...
	public static String DeleteFlowTemplete(String flowNo) {
		...
		try {
			...
			BP.DA.Log
					.DefaultLogWriteLineError("Do Method DelFlow Branch has a error , para:\t"
							+ flowNo + ex.getMessage());
			return ex.getMessage();
		}
	}
}