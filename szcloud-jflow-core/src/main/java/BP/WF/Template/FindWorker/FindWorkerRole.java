...
public class FindWorkerRole extends EntityOIDName {
	...
	public Map getEnMap() {
		...
		map.AddTBString(FindWorkerRoleAttr.Name, null, "Name", true, false, 0,
				200, 0);

		map.AddTBInt(FindWorkerRoleAttr.FK_Node, 0, "节点ID", false, false);

		// 规则存储.
		map.AddTBString(FindWorkerRoleAttr.SortVal0, null, "SortVal0", true,
				false, 0, 200, 0);
		map.AddTBString(FindWorkerRoleAttr.SortText0, null, "SortText0", true,
				false, 0, 200, 0);

		map.AddTBString(FindWorkerRoleAttr.SortVal1, null, "SortVal1", true,
				false, 0, 200, 0);
		map.AddTBString(FindWorkerRoleAttr.SortText1, null, "SortText1", true,
				false, 0, 200, 0);

		map.AddTBString(FindWorkerRoleAttr.SortVal2, null, "SortText2", true,
				false, 0, 200, 0);
		map.AddTBString(FindWorkerRoleAttr.SortText2, null, "SortText2", true,
				false, 0, 200, 0);

		map.AddTBString(FindWorkerRoleAttr.SortVal3, null, "SortVal3", true,
				false, 0, 200, 0);
		map.AddTBString(FindWorkerRoleAttr.SortText3, null, "SortText3", true,
				false, 0, 200, 0);

		// 规则采集信息值存储.
		map.AddTBString(FindWorkerRoleAttr.TagVal0, null, "TagVal0", true,
				false, 0, 1000, 0);
		map.AddTBString(FindWorkerRoleAttr.TagVal1, null, "TagVal1", true,
				false, 0, 1000, 0);
		map.AddTBString(FindWorkerRoleAttr.TagVal2, null, "TagVal2", true,
				false, 0, 1000, 0);
		map.AddTBString(FindWorkerRoleAttr.TagVal3, null, "TagVal3", true,
				false, 0, 1000, 0);

		// TagText
		map.AddTBString(FindWorkerRoleAttr.TagText0, null, "TagText0", true,
				false, 0, 1000, 0);
		map.AddTBString(FindWorkerRoleAttr.TagText1, null, "TagText1", true,
				false, 0, 1000, 0);
		map.AddTBString(FindWorkerRoleAttr.TagText2, null, "TagText2", true,
				false, 0, 1000, 0);
		map.AddTBString(FindWorkerRoleAttr.TagText3, null, "TagText3", true,
				false, 0, 1000, 0);
	...
	public final void DoUp() {
		this.DoOrderUp(FindWorkerRoleAttr.FK_Node,
				(new Integer(this.getFK_Node())).toString(),
				FindWorkerRoleAttr.Idx);
	}
	...
	public final void DoDown() {
		this.DoOrderDown(FindWorkerRoleAttr.FK_Node,
				(new Integer(this.getFK_Node())).toString(),
				FindWorkerRoleAttr.Idx);
	}
	...
	public final DataTable GenerWorkerOfDataTable() {
		...
		// ORIGINAL LINE: case "Leader":
		else if (this.getSortVal0().equals("Leader")
				|| this.getSortVal0().equals("SpecEmps")) {
			...
			case Submiter: // 当前提交人的直线领导
				empNo = BP.Port.WebUser.getNo();
				empDept = BP.Port.WebUser.getFK_Dept();
				break;
			case SpecNodeSubmiter: // 指定节点提交人的直线领导.
				sql = "SELECT FK_Emp,FK_Dept FROM WF_GenerWorkerlist WHERE WorkID="
						+ this.WorkID + " AND FK_Node=" + this.getTagVal1();
				dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
				if (dt.Rows.size() == 0) {
					throw new RuntimeException("@没有找到指定节点数据，请反馈给系统管理员，技术信息:"
							+ sql);
				}
				empNo = (String) ((dt.getValue(0, 0) instanceof String) ? dt
						.getValue(0, 0) : null);
				empDept = (String) ((dt.getValue(0, 1) instanceof String) ? dt
						.getValue(0, 1) : null);
				break;
			case BySpecField: // 指定节点字段人员的直接领导..
				sql = " SELECT " + this.getTagVal1() + " FROM "
						+ this.HisNode.getHisFlow().getPTable() + " WHERE OID="
						+ this.WorkID;
				dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
				empNo = (String) ((dt.getValue(0, 0) instanceof String) ? dt
						.getValue(0, 0) : null);
				if (StringHelper.isNullOrEmpty(empNo)) {
					throw new RuntimeException("@指定的节点字段(" + this.getTagVal1()
							+ ")的值为空.");
				}
				// 指定它
				Emp emp = new Emp();
				...
				break;
			default:
				throw new RuntimeException("@尚未处理的Case:"
						+ this.getHisFindLeaderType());
				// break;
			}
			if (StringHelper.isNullOrEmpty(empNo)) {
	...
	private DataTable GenerHisSpecEmps(String empNo, String empDept) {
		...
		case All: // 所有该部门性质下的人员.
			sql = "SELECT Leader FROM Port_DeptEmp WHERE FK_Emp='" + empNo
					+ "' AND FK_Dept='" + empDept + "'";
			dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
			leader = (String) ((dt.getValue(0, 0) instanceof String) ? dt
					.getValue(0, 0) : null);
			if (StringHelper.isNullOrEmpty(leader)) {
				throw new RuntimeException("@系统管理员没有给(" + empNo + ")在部门("
						+ empDept + ")中设置直接领导.");
			}
			break;
		case SpecDuty: // 特定职务级别的领导.
			...
			while (true) {
				sql = "SELECT FK_Emp FROM Port_DeptEmp WHERE DutyLevel='"
						+ this.getTagVal2() + "' AND FK_Dept='" + tempDeptNo
						+ "'";
...
//			break;
		case SpecStation: // 特定岗位的领导.
			...
			tempDeptNo = (String) ((tempVar2 instanceof String) ? tempVar2
					: null);
			while (true) {
...
//			break;
		case DutyLeader: // 特定职务的领导.
			...
			tempDeptNo = (String) ((tempVar2 instanceof String) ? tempVar2
					: null);
			while (true) {
				sql = "SELECT FK_Emp FROM Port_DeptEmp WHERE FK_Duty='"
						+ this.getTagVal2() + "' AND FK_Dept='" + tempDeptNo
						+ "'";
				DataTable mydt = DBAccess.RunSQLReturnTable(sql);
				if (mydt.Rows.size() != 0) {
...
//			break;
		case SpecStation: // 特定岗位的领导.
			...
			Object tempVar3 = new String(empDept);
			tempDeptNo = (String) ((tempVar3 instanceof String) ? tempVar3
					: null);
			while (true) {
				sql = "SELECT FK_Emp FROM Port_DeptEmpStation WHERE FK_Station='"
						+ this.getTagVal2()
