...
/**
 * 流程状态(详) ccflow根据是否启用草稿分两种工作模式,它的设置是在web.config 是 IsEnableDraft 节点来配置的. 1,
 * 不启用草稿 IsEnableDraft = 0. 这种模式下，就没有草稿状态, 一个用户进入工作界面后就生成一个Blank,
 * 用户保存时，也是存储blank状态。 2, 启用草稿.
 * 
 */
public enum WFState {
	/**
