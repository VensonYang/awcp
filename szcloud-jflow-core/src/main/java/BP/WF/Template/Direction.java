...
/**
 * 节点方向 节点的方向有两部分组成. 1, Node. 2, toNode. 记录了从一个节点到其他的多个节点. 也记录了到这个节点的其他的节点.
 */
public class Direction extends EntityMyPK {
	...
	public Map getEnMap() {
		...
		map.setEnDesc("节点方向信息");

		//
		// * MyPK 是一个复合主键 是由 Node+'_'+ToNode+'_'+DirType 组合的. 比如: 101_102_1
		//
		map.AddMyPK();
		map.AddTBString(DirectionAttr.FK_Flow, null, "流程", true, true, 0, 3, 0,
				false);
		...
		map.AddTBInt(DirectionAttr.IsCanBack, 0, "是否可以原路返回(对后退线有效)", false,
				true);
		//
		// * Dots 存储格式为: @x1,y1@x2,y2
		//
		map.AddTBString(NodeReturnAttr.Dots, null, "轨迹信息", true, true, 0, 300,
				0, false);
	...
	protected boolean beforeInsert() {
		this.setMyPK(this.getFK_Flow() + "_" + this.getNode() + "_"
				+ this.getToNode() + "_" + this.getDirType());
