...
public class LoadTemplete extends Method {
	...
	public LoadTemplete() {
		...
		this.Help += "@这些模板的位于" + SystemConfig.getPathOfData() + File.separator + "FlowDemo" + File.separator;
	}
	...
	public Object Do() {
		...
		String frmPath = SystemConfig.getPathOfData() + File.separator + "FlowDemo" + File.separator + "Form"
				+ File.separator;
			...
			for (File f : fls) {
				...
				try {
					...
					throw new RuntimeException(
							"@装载模版文件:" + f + "出现错误," + ex.getMessage() + " <br> " + ex.getStackTrace());
				} catch (Exception e) {
					// TODO 自动生成的 catch 块
					e.printStackTrace();
				}
		...
		dirInfo = new File(
				SystemConfig.getPathOfData() + File.separator + "FlowDemo" + File.separator + "Flow" + File.separator);
		...
		for (File dir : dirs) {
			...
			// 调度它的下一级目录.
			File dirSubInfo = new File(SystemConfig.getPathOfData() + File.separator + "FlowDemo" + File.separator
					+ "Flow" + File.separator + dir.getName());
			...
			for (File mydir : myDirs) {
				...
				for (File filePath : myfls) {
					msg += "@开始调度流程模板文件:" + filePath;
					Flow myflow = Flow.DoLoadFlowTemplate(subFlowSort.getNo(), filePath.getName(),
							ImpFlowTempleteModel.AsTempleteFlowNo);
					msg += "@流程:" + myflow.getName() + "装载成功。";
