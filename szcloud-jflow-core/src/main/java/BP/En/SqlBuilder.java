...
public class SqlBuilder {
	...
	public static String GenerFormWhereOfOra_For9i(Entity en) {
		...
		for (Attr attr : en.getEnMap().getAttrs()) {
			...
			if (attr.getMyFieldType() == FieldType.FK || attr.getMyFieldType() == FieldType.PKFK) {
				...
				// where=where+" AND "+"
				// ("+en.EnMap.PhysicsTable+"."+attr.Field+"="+en1.EnMap.PhysicsTable+"_"+attr.Key+"."+en1.EnMap.Attrs.GetFieldByKey(attr.UIRefKeyValue
				// )+" ) " ;
				continue;
			}
			if (attr.getMyFieldType() == FieldType.Enum || attr.getMyFieldType() == FieldType.PKEnum) {
				// from= from+ " LEFT OUTER JOIN "+table+" AS "+tableAttr+ " ON
				// "+enTable+"."+attr.Field+"="+tableAttr+"."+en1.EnMap.Attrs.GetFieldByKey(
				// attr.UIRefKeyValue );
				...
				from = from + " LEFT OUTER JOIN ( SELECT Lab, IntKey FROM Sys_Enum WHERE EnumKey='"
						...
						+ "=" + tableAttr + ".IntKey ";
				// where=where+" AND ( "+en.EnMap.PhysicsTable+"."+attr.Field+"=
	...
	// case MSSQL:
	// return "SELECT OID,FlowEndNode FROM " + table
	// + " WHERE GuestNo=" + SystemConfig.getAppCenterDBVarStr() + "GuestNo AND
	// WFState="
	// + SystemConfig.getAppCenterDBVarStr() + "WFState ";
	// case MySQL:
	// return "SELECT OID,FlowEndNode FROM " + table
	// + " WHERE GuestNo=" + SystemConfig.getAppCenterDBVarStr() + " AND
	// WFState="
	...
	public static String GenerFormWhereOfMS(Entity en) {
		...
		for (Attr attr : fkAttrs) {
			...
			if (attr.getIsFK()) {
				...
				from += " LEFT JOIN " + attr.getUIBindKey() + " AS " + attr.getUIBindKey() + "_" + attr.getKey()
						+ " ON " + mytable + "." + attr.getField() + "=" + attr.getUIBindKey() + "_" + attr.getField()
	...
	public static String SelectSQLOfMySQL(Entity en, int topNum) {
		...
		for (Attr attr : en.getEnMap().getAttrs()) {
			...
			case DataType.AppBoolean:
				...
				break;
			case DataType.AppDouble:
				val = val + ", IFNULL( round(" + mainTable + attr.getField() + " ,4) ,"
	...
	public static Paras GenerParas(Entity en, String[] keys) {
		...
		for (Attr attr : map.getAttrs()) {
													...
													// 是64位的int类型的数值问题.
				{
					...
					if (IsEnableNull) {
						...
						if (StringHelper.isNullOrEmpty(s)) {
							ps.Add(attr.getKey(), 0);
							// ps.AddDBNull(attr.getKey());//, DBNull.Value);
						} else {
							ps.Add(attr.getKey(), en.GetValIntByKey(attr.getKey()));
						}
					} else {
			...
			case DataType.AppDateTime:
				...
				// TODO modify by venson：rdt/cdt时间问题
				if (key.equals("rdt") || key.equals("cdt")) {
					da = sdf.format(new Date());
				} else {
					try {
						da = sdf.format(sdf.parse(da));
					} catch (Exception e) {
