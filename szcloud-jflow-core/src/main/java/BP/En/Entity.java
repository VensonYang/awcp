...
public abstract class Entity extends EnObj {
	...
	public final DataTable ToDataTableField(String tableName) {
		...
		for (Attr attr : this.getEnMap().getAttrs()) {
			...
			if (attr.getMyFieldType() == FieldType.FK || attr.getMyFieldType() == FieldType.PKFK) {
				dr.put(attr.getKey(), this.GetValByKey(attr.getKey()).toString().trim());
				/*
	...
	public int Retrieve() {
		...
		if (this.getEnMap().getDepositaryOfEntity() == Depositary.Application) {
			...
			if (SystemConfig.getIsDebug()) {
				throw new RuntimeException("@在[" + this.getEnDesc() + this.getEnMap().getPhysicsTable() + "]中没有找到["
						+ attr.getField() + attr.getDesc() + "]=[" + this.getPKVal() + "]的记录。");
			} else {
				throw new RuntimeException(
						"@在[" + this.getEnDesc() + "]中没有找到[" + attr.getDesc() + "]=[" + this.getPKVal() + "]的记录。");
			}
		}
		// C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		/// #endregion 从缓存里获取.
	...
	private boolean CheckDB() {
		...
		// {
		// string sql="DELETE "+en.RefTBName+" WHERE "+en.RefTBFK+"
		...
		for (EnDtl dtl : this.getEnMap().getDtls()) {
			...
			// DBAccess.RunSQL(sql);
			//
			// //string sql="SELECT "+dtl.RefKey+" FROM
			// "+dtl.Ens.GetNewEntity.EnMap.PhysicsTable+" WHERE "+dtl.RefKey+"
			// ='"+this.PKVal.ToString() +"' ";
			// DataTable dt= DBAccess.RunSQLReturnTable(sql);
			// if(dt.Rows.Count==0)
			// continue;
	...
	public final void DeleteHisRefEns() throws Exception {
		...
		// foreach(CheckData en in ens)
		// {
		// string sql="DELETE FROM "+en.RefTBName+" WHERE "+en.RefTBFK+"
		// ='"+this.GetValByKey(en.MainTBPK) +"' ";
		// DBAccess.RunSQL(sql);
		// }
		// C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	public final int Update(String key1, Object val1, String key2, Object val2, String key3, Object val3, String key4,
			Object val4, String key5, Object val5, String key6, Object val6) {
		this.SetValByKey(key1, val1);
		this.SetValByKey(key2, val2);
		this.SetValByKey(key3, val3);
		this.SetValByKey(key4, val4);
	...
	private void CreateIndexAndPK() {
		...
		/// #region 建立主键
		try {
			if (!DBAccess.IsExitsTabPK(this.getEnMap().getPhysicsTable())) {
				try {
					int pkconut = this.getPKCount();
					if (pkconut == 1) {
						try {
						...
						try {
							String pk0 = this.getPKs()[0];
							String pk1 = this.getPKs()[1];
							DBAccess.CreatePK(this.getEnMap().getPhysicsTable(), pk0, pk1,
									this.getEnMap().getEnDBUrl().getDBType());
							DBAccess.CreatIndex(this.getEnMap().getPhysicsTable(), pk0, pk1);
			...
			case DataType.AppDateTime:
				if (FType.toLowerCase().contains("char")) {
					...
					for (DataRow dr : dtYueShu.Rows) {
						...
						/*
						 * warning if
						 * (dr["FName"].toString().toLowerCase().equals(attr.
						 * getKey().toLowerCase())) {
						 * DBAccess.RunSQL("alter table " + table +
						 * " drop constraint " + dr[0].toString()); }
						 */
			...
			if (pkconut == 1) {
				...
				try {
					...
					try {
						...
						logger.debug("WF_GenerWorkerlist的索引已经存在");
					}
				} catch (java.lang.Exception e3) {
				}
			} else if (pkconut == 4) {
	...
	private void CheckPhysicsTable_Informix() throws Exception {
		...
		for (Attr attr : attrs) {
			...
			sql = "SELECT DATA_TYPE FROM ALL_TAB_COLUMNS WHERE  TABLE_NAME='"
					+ this.getEnMap().getPhysicsTableExt().toLowerCase() + "' AND COLUMN_NAME='"
					...
					for (String str : strsFK) {
						...
						if (ss[0].equals("AttrKey")) {
							...
							if (tempV.equals("") || tempV == null) {
								...
								// throw new Exception("@自动获取值信息不完整,Map
								// 中已经不包含Key=" + ss[1] + "的属性。");
