...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	*/
	...
	public final void Directly(String fromSQL, String toPTable, String pk1, String pk2)
	{
		this.Directly(fromSQL, toPTable);
		this.ToDBUrlRunSQL("CREATE INDEX "+toPTable+"ID ON "+toPTable+" ("+pk1+","+pk2+")");
	}
	public final void Directly(String fromSQL, String toPTable, String pk1, String pk2, String pk3)
	{
		this.Directly(fromSQL, toPTable);
	...
	public final void Directly(String fromSQL, String toPTable)
	{
		...
		for(DataColumn dc : dt.Columns)
		{
			...
			else if (dc.DataType.toString().equals("System.Int16") || dc.DataType.toString().equals("System.Int32") || dc.DataType.toString().equals("System.Int64"))
			{
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		for(DataRow FromDR : FromDataTable.Rows)
		{
			...
			for(FF ff : this.FFs)
			{
				switch(ff.DataType)
				{
					...
					case DataType.AppString:
						...
						/*
						 * warning UpdateSQL+= ff.ToField+ "='"+FromDR[ff.FromField].toString()+"',";*/
						break;
			...
			for(FF ff : this.FFs)
			{
				...
				/*
				 * warning UpdateSQL+= ff.ToField +"='"+FromDR[ff.FromField]+ "' AND ";*/
			}
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		for(DataRow ToDR : ToDataTable.Rows)
		{
			...
			for(FF ff : this.FFs)
			{
				...
				if(ff.DataType==DataType.AppDateTime)
				{
					SELECTSQL+=" case "+ " when datalength( CONVERT(NVARCHAR,datepart(month,"+ff.FromField+" )))=1 then datename(year,"+ff.FromField+" )+'-'+('0'+CONVERT(VARCHAR,datepart(month,"+ff.FromField+" ))) "+ " else "+ " datename(year,"+ff.FromField+" )+'-'+CONVERT(VARCHAR,datepart(month,"+ff.FromField+" )) "+ " END "+ "='"+ToDR.getValue(ff.ToField).toString()+"' AND ";
					/*
					 * warning SELECTSQL+=" case "+ " when datalength( CONVERT(NVARCHAR,datepart(month,"+ff.FromField+" )))=1 then datename(year,"+ff.FromField+" )+'-'+('0'+CONVERT(VARCHAR,datepart(month,"+ff.FromField+" ))) "+ " else "+ " datename(year,"+ff.FromField+" )+'-'+CONVERT(VARCHAR,datepart(month,"+ff.FromField+" )) "+ " END "+ "='"+ToDR[ff.ToField].toString()+"' AND ";*/
				}
				else
				{
					if(ff.DataType==DataType.AppString)
					{
						...
						/*
						 * warning SELECTSQL+=ff.FromField+"='"+ToDR[ff.ToField].toString()+"' AND ";*/
					}
					else
					{
						SELECTSQL+=ff.FromField+"="+ToDR.getValue(ff.ToField).toString()+" AND ";
			...
			if(result!=1)
			{
				...
				for(FF ff : this.FFs)
				{
					...
					if(ff.DataType==DataType.AppString)
					{
						...
						/*
						 * warning DeleteSQL+=ff.ToField+"='"+ToDR[ff.ToField].toString()+"' AND ";*/
					}
					else
					{
						DeleteSQL+=ff.ToField+"="+ToDR.getValue(ff.ToField).toString()+" AND ";
