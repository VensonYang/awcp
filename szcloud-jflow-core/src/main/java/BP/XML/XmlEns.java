...
public abstract class XmlEns extends ArrayList<XmlEn> {
	...
	private DataTable GetTableTxts(List<File> fis) {
		...
		DataTable cdt = (DataTable) ((tempVar instanceof DataTable) ? tempVar : null);
		if (cdt != null) {
			return cdt;
		}
	...
	private DataTable GetTableTxt(DataTable dt, File file) {
		...
		// java.util.Hashtable<String,Object> ht=new
			...
			try {
				...
				if (dt.Columns.size() == 0) {
					...
					try {
						...
						throw new RuntimeException("可能是没有在" + fi.getName() + "文件中找到表:" + this.getTableName()
				...
				throw new RuntimeException("获取数据出现错误:fileName=" + fi.getName() + " clasName=" + this.toString()
	...
	public final int Retrieve(String key, Object val, String key1, String val1) {
		...
		DataTable dt = this.GetTable();
		if (dt == null) {
			throw new RuntimeException("@错误：类" + this.getGetNewEntity().toString() + " File= " + this.getFile()
					+ " Table=" + this.getTableName() + " ，没有取到数据。");
		}
		...
		try {
			...
			throw new RuntimeException(
					"@装载错误:file=" + this.getFile() + " xml:" + this.toString() + "Err:" + ex.getMessage());
		}
		return i;
	}
	...
	public final XmlEn getItem(String key, String val) {
		...
		throw new RuntimeException("在[" + this.getTableName() + "," + this.getFile() + "," + this.toString()
				+ "]没有找到key=" + key + ", val=" + val + "的实例。");
	}
	...
	private List<File> listFile(List<File> lstFileNames, File f, String suffix, boolean isdepth) {
		// 若是目录, 采用递归的方法遍历子目录
		if (f.isDirectory()) {
			File[] t = f.listFiles();
