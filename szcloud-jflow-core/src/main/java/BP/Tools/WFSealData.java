	...
	//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
		...
		*/
		public final void UpdateColumn()
		{
			...
			{
				case Oracle:
					sql = String.format("ALTER TABLE %1$s modify(%2$s %3$s)", "Sys_WFSealData", WFSealDataAttr.SealData, "CLOB");
					break;
				case Informix:
				 //   DBAccess.RunSQL(SqlBuilder.GenerCreateTableSQLOfInfoMix(this));
					break;
				case MSSQL:
					sql = String.format("alter table %1$s alter column %2$s  %3$s", "Sys_WFSealData", WFSealDataAttr.SealData, "text");
					break;
				case MySQL:
					sql = String.format("ALTER TABLE %1$s MODIFY COLUMN %2$s %3$s", "Sys_WFSealData", WFSealDataAttr.SealData, "text");
					break;
				case Access:
		...
		*/
		...
		public Map getEnMap()
		{
			...
			map.AddTBString(WFSealDataAttr.FK_MapData, null, "FK_MapData", true, false, 0, 300, 20);
