...
import org.szcloud.framework.core.mybatis.sqlnode.OrderBySqlNode;
...
public class MapperProvider extends MapperTemplate {
	...
	public SqlNode select(MappedStatement ms) {
		return selectInternal(ms, false);
	}
	 ...
	 * @return
	 */
	public SqlNode selectWithCriterion(MappedStatement ms) {
		return selectInternal(ms, true);
	}

	private SqlNode selectInternal(MappedStatement ms, boolean withCriterion) {
		...
		if (withCriterion) {
			sqlNodes.add(new IfSqlNode(new StaticTextSqlNode(" DISTINCT "),
					"distinct"));
		}
		...
		if (withCriterion) {
			// 将if添加到<where>
			SqlNode condSqlNode = null;
			condSqlNode = createCriteria(ms);
			SqlNode ifSqlNode = new IfSqlNode(new WhereSqlNode(
					ms.getConfiguration(), condSqlNode), "_parameter != null");
			sqlNodes.add(ifSqlNode);

			// add order by
			SqlNode orderByNode = new IfSqlNode(new TextSqlNode(
					" ORDER BY ${orderByClause} "), "orderByClause != null");
			sqlNodes.add(orderByNode);

			// add limit
			SqlNode limitNode = new IfSqlNode(new StaticTextSqlNode(
					" LIMIT #{start},#{limit} "), "start !=0 or limit!=0");
			sqlNodes.add(limitNode);
		} else {
			sqlNodes.add(new WhereSqlNode(ms.getConfiguration(),
					getAllIfColumnNode(entityClass)));
			sqlNodes.add(new OrderBySqlNode(ms.getConfiguration(),
					getOrderByColumnNode(entityClass)));
		}
	...
	public SqlNode selectCount(MappedStatement ms) {
		return selectCountInternal(ms, false);
	}

	public SqlNode selectCountWithCriterion(MappedStatement ms) {
		return selectCountInternal(ms, true);
	}
	 ...
	 * @return
	 */
	private SqlNode selectCountInternal(MappedStatement ms,
		...
		sqlNodes.add(new StaticTextSqlNode(" COUNT(*) FROM "
				+ tableName(entityClass)));
		// 获取全部列的where,if条件
		if (withCriterion) {
			// 将if添加到<where>
	...
	public SqlNode delete(MappedStatement ms) {
		return deleteInternal(ms, false);
	}
	 ...
	 * @return
	 */
	public SqlNode deleteWithCriterion(MappedStatement ms) {
		return deleteInternal(ms, true);
	}
	 ...
	 * @return
	 */
	private SqlNode deleteInternal(MappedStatement ms, boolean withCriterion) {
		Class<?> entityClass = getSelectReturnType(ms);
		List<SqlNode> sqlNodes = new ArrayList<SqlNode>();
		// delete from table
